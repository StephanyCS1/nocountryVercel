...
export function Gallery() {
  ...
  return (
          <div className="flex flex-col gap-4 lg:gap-y-16">
            <GalleryRow title={'Cerca de vos'} cards={restaurants} isLoad={load} />
            <GalleryRow title={'Mejor calificados'} cards={restaurants} isLoad={load}/>
            {
              Object.values(user).length && user?.gustos.length 
              ?
              <GalleryRow title={'Segun tu preferencia'} cards={matchTastes || restaurants} isLoad={load}/>
              : 
